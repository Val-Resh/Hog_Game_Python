...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        first, last = opponent_score // 10, opponent_score % 10
        return 1 + max(first, last)
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    score, opponent_score = 0, 0 # Score for first player (0) and opponent_score for second player (1)

    def which_strategy(who):
        if who == 0: 
            return strategy0(score, opponent_score)
        else:
            return strategy1(opponent_score, score)

    who_score = lambda x: opponent_score if x == 0 else score

    while score < goal and opponent_score < goal:
        score_result = take_turn(which_strategy(who), who_score(who), select_dice(score, opponent_score))
        
        if who == 0:
            score += score_result
        else:
            opponent_score += score_result

        if score * 2 == opponent_score or opponent_score == score * 2:
            temp = score
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    num_rolls = 1
    while num_rolls != 11:
        current_avg = make_averaged(roll_dice, 10000)(num_rolls, dice)
        if current_avg > max_avg:
            max_avg = current_avg
            highest_roll = num_rolls
        print(num_rolls, 'dice scores', current_avg, 'on average')
        num_rolls +=1
    return highest_roll
...
def bacon_strategy(score, opponent_score):
    ...
    """
    first, last = opponent_score // 10, opponent_score % 10
    return 0 if 1 + max(first, last) >= BACON_MARGIN else BASELINE_NUM_ROLLS
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    swap = 1 + max(opponent_score // 10, opponent_score % 10)
    if swap == opponent_score / 2:
        return 0
    elif swap == opponent_score * 2:
...
def final_strategy(score, opponent_score):
    """Work in progress. I have yet to figure out a strategy that can result in a 
    60% winning rate.
    """
    if (score + opponent_score + max(opponent_score // 10, opponent_score % 10)) % 7 == 0:
        return 0
    else:
